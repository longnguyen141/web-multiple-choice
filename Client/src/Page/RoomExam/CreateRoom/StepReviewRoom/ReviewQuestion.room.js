...
const ReviewQuestions = () => {
    const { id } = useParams()
    const dataRedux = useSelector(state => state.room.infoExamRoom.listQuestion)
    const dataUpdateRedux = useSelector(state => state.updateRoom.infoExamRoom?.listQuestion)

    return (
        <div className="containerQuestionList">
            <div className="wrapQuestionList">
                {(id ? dataUpdateRedux : dataRedux)?.map((item, index) => (
